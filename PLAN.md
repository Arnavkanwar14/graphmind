...
**Dependency budget (ponytail rule):** backend = fastapi, uvicorn, psycopg, cryptography, pypdf, groq, python-multipart (FastAPI requires it to parse multipart uploads — no stdlib alternative), google-genai (Gemini fallback for extraction when Groq is rate-limited — a single free tier can't absorb a 400-doc bulk upload; 2026-07-12), psycopg_pool (per-request TLS handshakes to remote Neon added ~100-300ms to every endpoint — 2026-07-10 code review). Frontend = react, react-force-graph-2d, d3-force-3d (2026-07-13: promoted from a transitive dep of react-force-graph to a direct one — the graph layout is now precomputed synchronously with it, because react-force-graph's runtime simulation froze the 665-node graph in a rigid phyllotaxis circle and its force overrides only apply via an animation loop browsers throttle when unfocused). Anything beyond this list needs a reason written into this file.
