...
**Stack (decided, don't relitigate):** FastAPI (Python 3.10, own venv) serving a built React/Vite frontend from one process — one Render free-tier deploy, no CORS. Postgres on Neon free tier (Render's free disk is ephemeral, so nothing durable lives on it). Groq for all LLM calls (Arnav's free key). Postgres full-text search for retrieval in MVP — no embeddings/vector DB yet. Graph viz: hand-rolled canvas + d3-force-3d (2026-07-14, replacing react-force-graph-2d — see dependency budget note). Auth: email+password, `hashlib.scrypt` (stdlib — no passlib/bcrypt dep) + a `sessions` table with `secrets.token_hex` cookie (no JWT lib; revocation = DELETE row). File blobs encrypted with Fernet (`ENCRYPTION_KEY` env), stored as bytea in Postgres, 10 MB/file cap.

**Dependency budget (ponytail rule):** backend = fastapi, uvicorn, psycopg, cryptography, pypdf, groq, python-multipart (FastAPI requires it to parse multipart uploads — no stdlib alternative), google-genai (Gemini fallback for extraction when Groq is rate-limited — a single free tier can't absorb a 400-doc bulk upload; 2026-07-12), psycopg_pool (per-request TLS handshakes to remote Neon added ~100-300ms to every endpoint — 2026-07-10 code review). Frontend = react, d3-force-3d. react-force-graph-2d REMOVED 2026-07-14: its render loop died permanently on an internal exception during click/drag (confirmed by scripted reproduction — a drag that reliably zeroed the canvas to 0 painted pixels, with node positions and zoom transform both finite before and after, so it wasn't a data/NaN problem, the loop itself was crashing and nothing rescheduled a next frame). Replaced with a ~350-line hand-rolled canvas renderer: d3-force-3d simulation + a plain `<canvas>` + our own mousedown/mousemove/mouseup/wheel handlers for drag/pan/zoom, wrapped in try/catch so one bad frame can never kill the next. This is the standard force-graph idiom (same as D3's own official drag examples) — no wrapper library, no black-box render loop. Bonus: cut the JS bundle from ~367KB to ~206KB. Anything beyond this list needs a reason written into this file.
...
**Status (2026-07-11):** Stages 0–3 done and verified — live at https://graphmind-r439.onrender.com (Render free + Neon free). Auth, encrypted ingestion (tested with Baseten docs vault), and the knowledge graph all work end to end: batched Groq extraction (5 chunks/call, llama-3.3-70b) produces sane triples, Graph tab renders force-directed viz with entity detail panel. All 10 code-review findings fixed. UI reworked to match Retool reference (announcement bar, shimmer, motion, stats strip, dropzone). Stage 4 done too (2026-07-11): chat answers via entity-match → 1-hop traversal → chunk union with FTS (websearch + OR fallback), citations as clickable pills, "in the graph" chips jump to the Graph tab focused on the entity. Verified cross-document: "which engines support function calling?" answered correctly citing 3 docs. Stage 7 hardening partially done (2026-07-11): per-IP auth rate limit + per-user daily Groq budget (both in-memory, single-instance). CRLF chunker bug fixed same day. Stages 5+6 logic complete (2026-07-11): customers connect their own S3 (keys validated on connect, stored envelope-encrypted) or Google Drive (OAuth, refresh token encrypted; Google Docs exported as .txt); manual Sync pulls .md/.txt/.pdf through the shared ingest pipeline with sha dedupe. Verified to the boundary: fake S3 keys get AWS's real InvalidAccessKeyId surfaced cleanly; Drive returns a helpful 501 until GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET are set in Render (needs a GCloud OAuth client with redirect URI https://graphmind-r439.onrender.com/api/connectors/gdrive/callback — untested E2E until then; boto3 added to budget for S3). 2026-07-11 SaaS review: fixed failed-doc retry-on-reupload, account deletion (+danger-zone UI), Drive-connect feedback, memory/table housekeeping. **SaaS gaps needing Arnav's decision/accounts before real customers (chat history persistence now done 2026-07-12 — removed from this list):** email verification + a real transactional email provider (Resend free tier suggested; password reset already works, falls back to server-log link without a key), error monitoring (Sentry free tier), ToS/privacy pages, billing (Stripe), E2E connector test with real AWS/GCloud accounts, Render dashboard Start Command update for --proxy-headers (see 2026-07-11 sweep), Google OAuth client for Drive (GOOGLE_CLIENT_ID/SECRET in Render env). Working dir: `C:\Users\bifro\OneDrive\Desktop\graphmind`.
